...
export function startMockOpenCode(port = 0): Promise<MockOpenCode> {
  ...
  const send = (res: ServerResponse, obj: unknown) => res.write(`data: ${JSON.stringify(obj)}\n\n`);

  const messages: Record<string, Array<{ info: unknown; parts: unknown[] }>> = {};
  // Mutable session list so DELETE /session/:id removes the session (real server
  // behavior); GET /session reflects the current state.
  let sessions = [{ id: 'ses_mock', title: 'New session', slug: 'mock' }];
  ...
  const streamTurn = (sessionID: string) => {
    ...
    const D = (partID: string, delta: string) =>
      push({
        ...
        properties: { sessionID, messageID: 'm1', partID, field: 'text', delta },
      });
    ...
    P({
      ...
      state: { status: 'running', title: 'literature-search (OpenAlex)' },
    });
    P({
      id: 'c1',
      type: 'tool',
      callID: 'c1',
      tool: 'literature-search',
      state: { status: 'completed', title: 'literature-search (OpenAlex, PubMed)' },
    });
          ...
          providers: [
            {
              id: 'mock',
              name: 'Mock Provider',
              models: { 'mock-model': { name: 'Mock Model', limit: { context: 200_000 } } },
            },
          ],
        }),
      );
