  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        selector: {
          type: 'string',
          description: 'CSS 选择器，如 #submit-btn, .btn-primary, button[type=submit]',
        },
      },
      required: ['selector'],
    },
...
async function handleToolCall(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  switch (name) {
    ...
    case 'browser_open': {
      ...
      return {
        content: [
          { type: 'text', text: url ? `已打开浏览器面板并导航到: ${url}` : '已打开浏览器面板' },
        ],
      };
    }
    case 'browser_close': {
    ...
    case 'browser_record_save': {
      ...
      if (!steps || steps.length === 0) {
        return {
          content: [{ type: 'text', text: '没有可保存的录制。请先在浏览器面板点击录制按钮' }],
        };
      }
      ...
      return {
        content: [
          {
            type: 'text',
            text: `已保存录制 "${recName}"（${steps.length} 步）到 ${RECORDINGS_DIR}/${recName}.json`,
          },
        ],
      };
    }
    case 'browser_record_list': {
      ...
      for (const f of files) {
        try {
          ...
          lines.push(
            `- ${data.name}（${data.steps?.length ?? 0} 步，${data.created ?? '未知时间'}）${d}`,
          );
        } catch {
          /* skip invalid */
        }
    ...
    case 'browser_replay': {
        ...
        if (!existsSync(filePath)) {
          return {
            content: [
              {
                type: 'text',
                text: `录制 "${recName}" 不存在。使用 browser_record_list 查看可用录制`,
              },
            ],
          };
        }
      ...
      for (let i = 0; i < steps.length; i++) {
        ...
        try {
          ...
          results.push(
            `${i + 1}. ${step.method} ${step.path} — 失败: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
      ...
      return {
        content: [{ type: 'text', text: `回放完成（${steps.length} 步）:\n${results.join('\n')}` }],
      };
    }
      ...
      case 'initialize':
        return {
          ...
          result: {
            ...
            capabilities: { tools: {} },
            serverInfo: { name: 'workbench-browser', version: '1.0.0' },
          },
        };
      default:
