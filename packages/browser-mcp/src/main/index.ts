...
import { ipcMain, session, webContents, BrowserWindow } from "electron";
...
export function createBrowserMcp(opts: BrowserMcpOptions): BrowserMcpPlugin {
  ...
  function registerIpc() {
    ...
    ipcMain.handle("browser:record-state", () => ({
      isRecording, count: recordedSteps.length,
    }));
    ...
    ipcMain.handle("browser:record-save", (_e, name: string, description?: string) => {
      ...
      writeFileSync(join(dir, `${name}.json`), JSON.stringify({
        name, description: description ?? "", steps: lastRecording, created: new Date().toISOString(),
      }, null, 2));
      ...
      return readdirSync(dir)
        ...
        .map((f) => {
          try {
            ...
            return { name: d.name, steps: d.steps?.length ?? 0, created: d.created ?? "", description: d.description ?? "" };
          } catch { return null; }
        })
        .filter(Boolean);
    });
    ...
    ipcMain.handle("browser:record-replay", async (_e, name: string, delay = 500) => {
      ...
      const cmdMap: Record<string, string> = {
        navigate: "navigate", click: "click", "click-at": "click-at",
        type: "type-selector", select: "select", hover: "hover",
        scroll: "scroll", "execute-js": "execute-js", screenshot: "screenshot",
        "get-content": "get-content", "get-html": "get-html",
      ...
      for (let i = 0; i < steps.length; i++) {
        ...
        try {
          ...
          await sendBrowserCommand(cmdMap[cmd] ?? cmd, (step.body as Record<string, unknown>) ?? {});
          results.push(`${i + 1}. ${step.path} — 成功`);
        } catch (err) {
          results.push(`${i + 1}. ${step.path} — 失败: ${err instanceof Error ? err.message : String(err)}`);
        }
  ...
  function startHttpApi() {
    ...
    server = http.createServer(async (req: any, res: any) => {
      ...
      const shouldRecord = isRecording && req.url?.startsWith("/browser/") && !req.url.startsWith("/browser/record") && !req.url.startsWith("/browser/panel");
      ...
      try {
        ...
        if (req.method === "POST" && req.url === "/browser/record/start") {
          isRecording = true; recordedSteps = []; sendJson({ ok: true }); return;
        }
        if (req.method === "POST" && req.url === "/browser/record/stop") {
          isRecording = false; lastRecording = [...recordedSteps];
          sendJson({ ok: true, count: recordedSteps.length }); return;
        }
        if (req.method === "GET" && req.url === "/browser/record/state") {
          sendJson({ isRecording, count: recordedSteps.length }); return;
        }
        if (req.method === "GET" && req.url === "/browser/record/steps") {
          sendJson(lastRecording); return;
        }
        // Download
        ...
        const cmdMap: Record<string, string> = {
          navigate: "navigate", "execute-js": "execute-js", content: "get-content",
          html: "get-html", url: "get-url", title: "get-title",
          click: "click", "click-at": "click-at", type: "type-selector",
          select: "select", hover: "hover", scroll: "scroll", screenshot: "screenshot",
          back: "back", forward: "forward", refresh: "refresh",
        };
