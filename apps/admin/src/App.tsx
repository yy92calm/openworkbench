    ...
    return (
      <Centered>
        ...
        >
          ...
          {loginError && <p style={{ color: "var(--error)", fontSize: 13, marginBottom: 10 }}>{loginError}</p>}
          <button className="btn-primary" type="submit" disabled={!password} style={{ width: "100%" }}>
            <KeyRound size={15} />
            登录
          </button>
        </form>
    ...
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "32px 20px 64px" }}>
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button className="btn-ghost" onClick={onLogout} style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
            <LogOut size={13} />
            退出
          </button>
        </div>
      ...
      <div className="card" style={{ padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <div className="field" style={{ flex: "2 1 200px" }}>
            ...
            <input value={newToken} onChange={(e) => setNewToken(e.target.value)} placeholder="token" spellCheck={false} />
          </div>
          <div className="field" style={{ flex: "1 1 140px" }}>
            <label>备注（可选）</label>
            <input value={newNote} onChange={(e) => setNewNote(e.target.value)} placeholder="用户名 / 用途" />
          </div>
          <button
      ...
      {accounts === null ? (
        ...
        <p style={{ color: "var(--muted)", fontSize: 14 }}>还没有账号。用上面的表单添加第一个令牌。</p>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {accounts.map((acc) => (
            <div key={acc.token} className="card">
              ...
              >
                <span style={{ fontFamily: "ui-monospace, monospace", fontSize: 13, flex: "1 1 140px", wordBreak: "break-all" }}>
                  {acc.token}
                </span>
                {acc.note && <span style={{ fontSize: 12, color: "var(--muted)" }}>{acc.note}</span>}
                      ...
                      <tbody>
                        {acc.devices.map((d) => (
                          <tr key={d.device} style={{ fontSize: 13 }}>
                            <td style={{ padding: "7px 4px", fontFamily: "ui-monospace, monospace" }}>{d.device}</td>
