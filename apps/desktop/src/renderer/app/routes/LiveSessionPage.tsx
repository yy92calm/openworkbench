...
import { ArrowDown, FolderOpen, Loader2, NotebookPen, PlugZap } from 'lucide-react';
...
export function LiveSessionPage() {
  ...
  const onRunCommand = async (name: string, args: string) =>
    afterTurn(await runCommand(name, args));
  ...
  useEffect(() => {
    return window.electronAPI.on(
      'browser:panel',
      (_event: unknown, msg: { action: string; url?: string }) => {
        if (msg.action === 'open') {
                  ...
                  {working && (
                    ...
                    <div className="flex min-w-0 items-center gap-2.5 text-sm text-muted">
                      ...
                      {!activeRequest && currentTool && (
                        <span
                          className="truncate rounded bg-surface-2 px-1.5 py-0.5 font-mono text-[11px] text-text-dim"
...
function FilePreviewTab({
  ...
  return (
    <div className="h-full">
      <InspectorShell
        inspector={{ ...fileInspectorFromBlock(artifact), root }}
        onClose={onClose}
        onEvaluate={onEvaluate}
      />
    </div>
