...
function KvCard(payload: unknown, options?: Record<string, unknown>): ReactNode {
  ...
  return (
    <div className="my-3 overflow-hidden rounded-input border border-border bg-surface shadow-card">
      {label && (
        <div className="border-b border-border bg-surface-2 px-4 py-2 text-[13px] font-medium text-text">{label}</div>
      )}
...
export function renderWorkbenchFence(type: string, raw: string, enabled: Map<string, RendererManifest>): ReactNode | null {
