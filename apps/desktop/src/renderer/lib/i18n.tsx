...
const en: TranslationMap = {
  ...
  'settings.remote.note': 'The token never leaves this machine.',
  'settings.themeLight': 'Light',
  'settings.themeDark': 'Dark',
  'settings.themeSystem': 'System',
  'settings.expandDetails': 'Expand reasoning & tools by default',
  'settings.expandDetailsHint':
    'When off, reasoning and tool calls start collapsed; click to expand each',
  ...
  'settings.dataFlow.local2':
    'Code and terminal run locally - the Python/R kernel and shell execute here; datasets are never uploaded in bulk.',
  'settings.dataFlow.local3':
    "Session history, scheduled-task logs, and a traceable record of every artifact, in the app's private folder.",
  'settings.dataFlow.local4':
    'Provider keys and login tokens - a file only you can read; never written to the workspace, logs, or exports.',
  'settings.dataFlow.sent': 'Sent to your model provider',
  'settings.dataFlow.sent1':
    'Your messages, and the file contents / command output the agent reads to do what you asked.',
  'settings.dataFlow.sent2':
    'Data leaves only during a conversation turn - including turns triggered by a scheduled task.',
  'settings.dataFlow.sent3': 'What the provider keeps is governed by its own privacy policy.',
  'settings.dataFlow.footnote':
    'Bundled skills and MCP servers (e.g. web fetch) may make their own network calls - review the .opencode profile before packaging.',
...
const zhCN: TranslationMap = {
  ...
  'settings.remote.note': '令牌只保存在本机，不会发送给客户端。',
  ...
  'settings.dataFlow.local2':
    '代码与终端在本地运行--Python/R 内核与 shell 就地执行，数据集不会批量上传。',
  ...
  'settings.dataFlow.footnote':
    '内置技能与 MCP 服务器（如网页抓取）可能自行发起网络请求--打包前请检查 .opencode 配置。',
...
export function I18nProvider({ locale, children }: { locale: Locale; children: ReactNode }) {
  const t = useCallback((key: string) => maps[locale]?.[key] ?? key, [locale]);
