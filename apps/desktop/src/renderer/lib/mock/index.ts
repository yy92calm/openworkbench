...
const exampleAnalysis: Session = {
  ...
  blocks: [
    { kind: 'user', text: '分析我添加的销售数据，找出趋势并生成一份报告' },
    {
      ...
      summary: '完成 5 个步骤',
      steps: 5,
      details: ['读取 sales.csv', '清洗缺失值', '按月份聚合', '生成图表', '撰写 report.md'],
    },
    {
      kind: 'tool-call',
      title: '运行数据脚本',
      status: 'success',
      meta: '12s',
      inputSummary: 'sales.csv',
      outputSummary: '聚合完成，共 24 行',
    },
    {
      kind: 'agent',
      markdown:
...
const exampleBuild: Session = {
  ...
  blocks: [
    ...
    },
    {
      ...
      title: '试运行 rename.py',
      status: 'success',
      shellCommand: 'python rename.py --dry-run',
      meta: '2s',
      outputSummary: '将重命名 12 个文件',
    },
    {
      kind: 'agent',
      markdown: '完成。脚本 `rename.py` 已保存并验证，支持 `--dry-run` 预览，可安全批量执行。',
    },
    {
      kind: 'artifact',
      path: 'rename.py',
...
const exampleTrace: Session = {
  ...
  blocks: [
    { kind: 'user', text: '帮我核对“大客户流失”这个结论的依据是否站得住。' },
    {
      ...
      status: 'success',
      meta: '8s',
      outputSummary: '按客户分组统计续约率',
    },
    {
      kind: 'table',
      columns: ['客户', '续约率', '变动', '依据'],
      rows: [
        ['客户A', '92%', '+3%', '近3个季度'],
        ['客户B', '41%', '-19%', 'Q3 流失'],
        ['客户C', '78%', '持平', '自动续约'],
...
const exampleSessions: Session[] = [exampleAnalysis, exampleBuild, exampleTrace];
