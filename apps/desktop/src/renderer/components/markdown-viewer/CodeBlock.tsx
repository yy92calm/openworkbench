...
export function useCopyCode(text: string) {
...
export type Variant = 'chat' | 'document';

export const STYLES: Record<Variant, Record<string, string>> = {
  chat: {
  ...
  document: {
    ...
    blockquote:
      'my-5 rounded-r-md border-l-[3px] border-[#d98c6a] bg-[#faf6f2] py-1.5 pl-5 pr-4 text-[#6b6155] [&_p]:my-1.5',
...
export const CodeBlock = memo(function CodeBlock({
  ...
  const run = async () => {
    ...
    try {
      ...
      // Kernel output may carry ANSI colors; this is a plain-text spot, so the
      // escapes are stripped at display time (raw bytes stay untouched).
      setOutput(res ? stripAnsi(formatExecResult(res)) : '(execution unavailable)');
    } catch {
