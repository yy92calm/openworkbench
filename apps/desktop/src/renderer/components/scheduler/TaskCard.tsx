...
export function TaskCard({
  ...
  return (
    <div className="rounded-card border border-border bg-surface">
      ...
      >
        <span
          className={cn('h-2 w-2 shrink-0 rounded-full', task.enabled ? 'bg-ok' : 'bg-muted')}
        />
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          <label
            className="relative ml-1 inline-flex cursor-pointer items-center"
      ...
      {expanded && (
        <div className="border-t border-border px-4 py-3 space-y-2">
          ...
          {task.agent && <div className="text-xs text-muted">Agent: {task.agent}</div>}
