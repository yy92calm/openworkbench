...
export class HostClient {
  ...
  async newDatedWorkspace(name: string): Promise<{ path: string }> {
    return jsonOrThrow(await this.fetch("/__host/workspace/dated", {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  async updateTask(id: string, patch: UpdateTaskInput): Promise<ScheduledTask | null> {
    return jsonOrThrow(await this.fetch(`/__host/scheduler/tasks/${encodeURIComponent(id)}`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
  ...
  async deleteTask(id: string): Promise<void> {
    await jsonOrThrow(await this.fetch(`/__host/scheduler/tasks/${encodeURIComponent(id)}`, { method: "DELETE" }));
  }
  async toggleTask(id: string, enabled: boolean): Promise<ScheduledTask | null> {
    return jsonOrThrow(await this.fetch(`/__host/scheduler/tasks/${encodeURIComponent(id)}/toggle`, {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  async fireNow(id: string): Promise<ExecutionRecord | null> {
    return jsonOrThrow(await this.fetch(`/__host/scheduler/tasks/${encodeURIComponent(id)}/fire`, { method: "POST" }));
  }
