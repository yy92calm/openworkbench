      ...
      <div className="rooms-list">
        ...
        {recent.map((r) => (
          <div key={r.inviteCode} className="rooms-list-item">
            <button className="rooms-list-main" onClick={() => handlePickRecent(r.inviteCode)}>
    ...
    const unsub = onRoomEvent((e: RoomEvent) => {
      switch (e.type) {
        ...
        case 'message': {
          ...
          const text =
            kind === 'text' || kind === 'session-share' ? decodeMessage(m.ct, m.nonce) : '';
  ...
  return (
    <div style={{ height: '100%', minHeight: 0, display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="room-header">
        <button onClick={handleLeave} className="room-back">
          ←
        </button>
        <div style={{ flex: 1 }}>
          ...
          <div key={m.messageId} className={`room-msg ${m.fromMe ? 'mine' : ''}`}>
            ...
            {m.viewOnce && !m.fromMe ? (
              ...
              <>
                ...
                {m.fromMe && m.read && <div className="room-msg-read">已读</div>}
              </>
            )}
      ...
      <div className="room-input-wrap">
        ...
        <div className="room-input-bar">
          ...
          <input
            ...
            placeholder={
              enforceViewOnce || viewOnceMode ? '阅后即焚消息…' : '输入消息…（/ 分享历史会话）'
            }
          ...
          />
          {recording ? (
            <button className="room-record-btn recording" onClick={stopRecording} title="停止录音">
              ● {recordSeconds}″
            </button>
          ) : (
            <button
  ...
  return (
    <div className="room-share-picker">
      <div className="room-share-header">
        <span>分享历史会话</span>
        <button onClick={onClose} className="room-share-close" aria-label="关闭">
          ×
        </button>
      </div>
  ...
  return (
    <div className={`room-msg-share ${open ? 'open' : ''}`}>
      <button className="room-msg-share-head" onClick={() => setOpen((v) => !v)}>
        ...
        <span className="room-msg-share-title">
