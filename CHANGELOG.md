...
- 申万行业分析（方案 docs/20260922-01）：新增「申万行业 · 一级全景」——
  申万宏源官网公开接口（免 key，akshare 同源）接入 31 个一级行业：实时
  行情（涨跌、成交额占比）+ 逐日分析（截止最近完整日：PE / PB / 股息率 /
  换手 / 流通市值），并复用透明评分口径（申万一级行业内百分位）给出 60 日
  动量、相对沪深300、评分 / 信号与较上一交易日变化；表格限高滚动、行点击
  看 100 日走势与明细（引用到对话 / 交给对话解析）；轮动日报与周报 prompt
  注入申万快照（评分降序 + 估值 + 成交占比）；中证十行业与申万一级两个
  粒度并置。
- CEO 展示风格（方案 docs/20260922-02）：申万区块新增结论两行（评分领先 /
  垫底 + 成交聚焦，`buildSwConclusion`）；申万表按信号分组（超配 / 中性 /
  低配 / 未评分 + 数量）并加评分条（随信号配色）、成交额占比迷你条、今日
  涨跌色块 chip；轮动表评分列加同款比例条、趋势改为语义色圆点；台账统计行
  加归因率闭环进度条；次要数据列统一降噪（估值 / 换手等 11px muted）。
- 行业模型热力图（方案 docs/20260923-01）：行业模型区由「搜索单个板块」改为
  **全板块热力图**——squarified treemap，方块面积 ∝ 东财板块总市值、红涨
  绿跌（深浅随幅度，市值缺失给可见小方块）；点击方块弹窗查看全套明细
  （换手 / 资金 / 市值 / PE 分位 + 120 日走势 + 成分股 TOP10 + 记录决策 /
  交给对话解析）；移除搜索框与内嵌面板，详情改为按需拉取。

### Fixed（宏观洞察数据源，方案 docs/20260923-02）

- 申万官网接口只发 leaf 证书（缺中间证书），Node fetch 直接拒绝
  （`UNABLE_TO_VERIFY_LEAF_SIGNATURE`）——宏观数据抓取改用 Electron
  `net.fetch`（Chromium 网络栈：自动补链、遵循系统代理），实测申万 31 个
  行业恢复；
- 行情类接口（push2）存在间歇性丢连接（`Empty reply from server`）：抓取
  自动重试（默认 3 次、退避 500/1000ms；1MB 级响应 2 次），实测 7/7 数据源
  就绪；
- 错误信息带底层原因（`fetch failed：other side closed (UND_ERR_SOCKET)`），
  便于区分网络与解析问题；
- 板块列表（东财 `clist` 端点）被出口网络封锁时，行业模型区显示明确提示
  （「板块列表数据源暂不可用；行情与轮动不受影响，稍后自动重试」）。
- 顶部少字 + 行业标签（方案 docs/20260923-03）：删除定位语；「建议超配 /
  建议低配」由句子改为超配 / 低配两行行业标签（中证十行业，信号色 pill +
  中性 chip）；「较上一交易日」保留一行；5 格 KPI 去掉副文案；导出 / 复制 /
  打印仍使用完整句子与 KPI 表（`buildMacroSignalView` 纯函数，导出复用）。
