...
- `macroData.ts`：公开接口纯解析（指数行情、日 K、宏观报表、中美国债
  收益率、基金排行文本、行业板块列表含板块 PE、板块成分股），输入容错。
- `macroStore`（`macro.ts`）：按 indices / yields / macro / fx / funds /
  industries 六组并行抓取；内存快照 + `userData/macro-cache.json` 磁盘缓存
  （冷启动秒开）；5 分钟 TTL、并发去重、按需历史（`macro-series`）与
  行业详情（`macro-industry`，成分股 PE/PB 中位数 + 板块 PE 分位 + 120 日
...
- 行业模型：东财细分行业（约 496 个，按市值取 TOP100 并做多级命名去重）
  搜索；选中后拉成分股 TOP20 估值与板块历史；板块 PE 在 TOP100 板块截面
  的百分位展示（仅正 PE，越高越贵）。
...
- 定时任务：首次启动一次性自动开通轮动日报（工作日 08:30）/ 轮动周报
  ...
  （删除不复活）。
- 渲染层：`/macro` 页面（汇报概览：结论句 + 5 格 KPI；后台简报区：每个
  主题展示最近报告，可查看 / 复制 / 在对话中打开 / 重新生成；轮动评分表 +
  行业面板 + 决策台账，数据底座折叠）；界面动作只有「重新生成（后台任务
  立即跑一次）」与「交给对话解析（带上下文开新会话）」，其余研究操作
  （记录 / 筛选 / 归因 / 引用等）默认隐藏、悬停显现，阅读态只保留
  刷新 / 导出 / 通知；汇报摘要可复制到剪贴板、导出 Markdown 到
  `<workspace>/.workbench/research/`、打印（打印藏非打印元素、自动切浅色
  主题）；红涨绿跌（`rise`/`fall` 语义色）；通知经 `macro-notification`
  推送 + 侧栏未读红点。
