...
| `/macro` | `MacroInsightsPage` | 宏观洞察：汇报概览（超配 / 低配行业标签两行 + 较上一交易日一行 + 5 格 KPI + 导出菜单）+ 后台简报（自动生成报告：查看 / 复制 / 在对话中打开 / 重新生成）+ 轮动模型 + 申万一级行业全景（31 行，行情 / 估值 / 评分，行点击看走势明细）+ 行业模型（全板块热力图：面积 = 市值、红涨绿跌，点击弹窗看明细）+ 决策台账；数据底座折叠、研究操作悬停显现（模型区只有「重新生成 + 交给对话解析」），汇报摘要可复制 / 导出 / 打印 |
