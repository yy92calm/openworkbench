  ...
  const server = createServer((req, res) => {
    ...
    if (req.method === "POST" && m) {
      ...
      setTimeout(() => {
        ...
        push({ type: "message.part.updated", properties: { part: { sessionID, id: "p1", type: "text", text: "" } } });
        push({ type: "message.part.delta", properties: { sessionID, partID: "p1", field: "text", delta: "Planning " } });
        push({ type: "message.part.delta", properties: { sessionID, partID: "p1", field: "text", delta: "works." } });
        push({ type: "message.part.updated", properties: { part: { sessionID, id: "p1", type: "text", text: "Planning works." } } });
...
class MiniOpenCodeClient {
    ...
    for (;;) {
      ...
      while ((idx = buf.indexOf("\n\n")) >= 0) {
        ...
        try {
          ...
          if (ev.type === "message.part.updated" && ev.properties?.part?.type === "text") this.sawText = true;
  ...
  async deleteSession(sessionId) {
    const res = await this.fetchImpl(`${this.baseUrl}/session/${encodeURIComponent(sessionId)}`, { method: "DELETE" });
...
client.close();
await connectDone.catch(() => { /* 长连接随 relay 关闭自然结束 */ });
t.close();
host.close();
await relay.close();
await mock.close();
