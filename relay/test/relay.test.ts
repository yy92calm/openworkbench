  ...
  ws.on("message", (data) => {
    ...
    void (async () => {
      ...
      if (opts.direct || sidecarPort == null) {
        ...
        ws.send(JSON.stringify({ type: "head", id: msg.id, status: 200, headers: {} } satisfies RelayMessage));
...
describe("RelayServer + RelayHttpTransport", () => {
  ...
  it("rejects guest pairing with a device not registered under the account", async () => {
    ...
    await expect(guest.fetchImpl("http://relay/echo")).rejects.toThrow(/relay not connected|connection closed/);
  });
  ...
  it("rejects listAccountDevices with a wrong token", async () => {
    const { relayUrl } = await setup();
    await expect(listAccountDevices(relayUrl, "nope", { WebSocketImpl: WebSocket })).rejects.toThrow();
  });
  ...
  it("persists the account registry to disk and loads it back", async () => {
    ...
    const { relayUrl, sidecarPort, relay } = await setup({ dataDir: dir });
  ...
  it("hot-reloads account removal: admin CLI removes an account and the live relay kicks its connections", async () => {
    ...
    const { relayUrl, relay } = await setup({ dataDir: dir });
    const host = await startFakeHost(relayUrl);
...
describe("Admin API", () => {
  ...
  it("creates, lists (with online status), removes accounts and devices", async () => {
    ...
    const host = new WebSocket(`${base.replace(/^http:/, "ws:")}?role=host&device=dev-a&token=tok-api`);
    await new Promise<void>((r) => {
      host.on("open", () => r());
    });

    const accounts = await (await authed("/accounts")).json() as { accounts: Array<{ token: string; devices: Array<{ device: string; online: boolean }> }> };
    ...
    const host2 = new WebSocket(`${base.replace(/^http:/, "ws:")}?role=host&device=dev-b&token=tok-api`);
    await new Promise<void>((r) => host2.on("open", () => r()));
    host2.close();
    await new Promise((r) => setTimeout(r, 30));

    const after = await (await authed("/accounts")).json() as { accounts: Array<{ token: string; devices: Array<{ device: string; online: boolean }> }> };
    const devB = after.accounts.find((a) => a.token === "tok-api")?.devices.find((d) => d.device === "dev-b");
    ...
    const afterDel = await (await authed("/accounts")).json() as { accounts: Array<{ token: string; devices: Array<{ device: string }> }> };
    const devList = afterDel.accounts.find((a) => a.token === "tok-api")?.devices.map((d) => d.device) ?? [];
    expect(devList).not.toContain("dev-b");

    // Delete the whole account.
    const del = await authed("/accounts/tok-api", { method: "DELETE" });
    expect(del.status).toBe(200);
    const finalL = await (await authed("/accounts")).json() as { accounts: Array<{ token: string }> };
