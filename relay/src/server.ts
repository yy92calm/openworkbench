...
export class RelayServer {
  ...
  private handleHttp(
    ...
    if ((url === '/relayadmin' || url.startsWith('/relayadmin/')) && this.opts.adminStaticDir) {
      this.handleStaticDir(
        req,
        res,
        this.opts.adminStaticDir,
        url.replace(/^\/relayadmin/, '') || '/',
      );
      return;
    }
    ...
    if (dl && req.method === 'GET') {
      ...
      if (file.meta.filename) {
        headers['content-disposition'] =
          `inline; filename="${file.meta.filename.replace(/"/g, '_')}"`;
      }
  ...
  private setAdminSession(res: import('node:http').ServerResponse, sid: string): void {
    ...
    res.setHeader(
      'set-cookie',
      `admin_session=${sid}; HttpOnly; SameSite=Strict; Path=/; Max-Age=${Math.floor(RelayServer.ADMIN_SESSION_TTL_MS / 1000)}${secure}`,
    );
  }
